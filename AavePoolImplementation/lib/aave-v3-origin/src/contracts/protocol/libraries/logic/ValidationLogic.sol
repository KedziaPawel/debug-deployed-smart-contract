...
library ValidationLogic {
    ...
    function validateBorrow(
        ...
        require(
            params.interestRateMode == DataTypes.InterestRateMode.VARIABLE, Errors.INVALID_INTEREST_RATE_MODE_SELECTED
        );
    ...
    function validateAutomaticUseAsCollateral(
        ...
        if (reserveConfig.getDebtCeiling() != 0) {
            ...
            if (
                !IAccessControl(addressesProvider.getACLManager()).hasRole(ISOLATED_COLLATERAL_SUPPLIER_ROLE, msg.sender)
            ) return false;
        }
